...
function _normalizeTimestamp(fact) {
  let ms = fact.timestampMs;
  if (typeof ms !== "number" || !Number.isFinite(ms) || ms < MIN_TIMESTAMP_MS || ms > MAX_TIMESTAMP_MS) {
    const parsed = Date.parse(fact.timestamp);
    if (Number.isFinite(parsed) && parsed >= MIN_TIMESTAMP_MS && parsed <= MAX_TIMESTAMP_MS) ms = parsed;
    else ms = Date.now();
  }
  const iso = new Date(ms).toISOString();
  // ensure timestamp string is valid ISO; regenerate if malformed
  const parsedIso = Date.parse(fact.timestamp);
  const timestamp = Number.isFinite(parsedIso) ? fact.timestamp : iso;
  return { timestamp, timestampMs: ms, isoMs: ms };
}
...
export async function loadMemory(path) {
  ...
  try {
    ...
    for (const f of parsed.facts) {
      ...
      // Phase 1.3: migrate timestampMs, generate if missing/invalid
      ...
      if (typeof timestampMs !== "number" || !Number.isFinite(timestampMs)) {
        ...
        // if original had no timestampMs, mark dirty for backfill (lazy persist on next write)
        if (f.timestampMs === undefined) dirty = true;
      }
    ...
    // lazy migration: if dirty, schedule background save? We keep dirty in memory but not auto-write to avoid storm.
    // Caller (store) will persist on next write; we could optionally trigger async save, but keep lazy.
    return { facts: sliced };
  } catch (error) {
